...
## Этап 1: Подключение зависимостей
...
## Этап 2: Описание маршрутов (Type-Safe Routes)
...
## Этап 3: Создание экранов (TaskListScreen и TaskDetailScreen)
...
- Создаём новый `TaskListScreen` (и переименование `TaskListViewModel`).
...
## Этап 4: Настройка NavHost в App.kt
...
- Описание графа навигации: `composable<TaskListRoute> { ... }` и `composable<TaskDetailRoute> { ... }`.
- Проброс обработчиков навигации (например, лямбды `onNavigateToTask`) в экраны.
- Не забываем про то, что смена темы должна работать на уровне всего приложения, и нужен hoisting для теперь уже `SettingsViewModel` 
- **[Ожидаем аппрув -> Выполняем `git commit`]**

## Этап 5: Слияние в основную ветку (Merge)
