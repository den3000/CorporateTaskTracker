...
## [ВЫПОЛНЕНО] Этап 5: Репозиторий и Интеграция с UI
...
## [ВЫПОЛНЕНО] Этап 5.1: Оптимизация Навигации (SSOT)
**Задача:** Перейти к паттерну Single Source of Truth, убрав передачу объектов через навигацию.
- Возврат к передаче только `taskId` в `TaskDetailRoute`.
- Реализация загрузки данных из БД во внутри `TaskDetailViewModel` по ID.
- Использование реактивного `Flow` в `TaskListViewModel` для автоматического обновления списка при сохранении задачи на другом экране.
- Удаление ручной сериализации данных при переходах между экранами.
- **[Ожидаем аппрув -> Выполняем `git commit`]**
